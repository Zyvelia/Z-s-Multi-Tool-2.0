...
class BreachCheckerPage(ctk.CTkFrame):
    ...
    def _build_password_tab(self):
        ...
        self.pw_entry = ctk.CTkEntry(
            ...
            show="\u2022", fg_color=theme.PANEL_2, corner_radius=8,
            text_color=theme.TEXT, border_width=0, height=36
        )
        ...
        self.pw_check_btn = _btn(btn_row, "Check Password", self.check_password_clicked, **theme.primary_button_kwargs())
        self.pw_check_btn.pack(side="left")

        self.pw_status = ctk.CTkLabel(
            btn_row, text="", font=("Segoe UI", 11), text_color=theme.MUTED
        )
        self.pw_status.pack(side="left", padx=(12, 0))

        result_card = ctk.CTkFrame(tab, fg_color=theme.PANEL_2, corner_radius=10)
        result_card.pack(fill="x", padx=14, pady=(16, 14))

        self.pw_result_title = ctk.CTkLabel(
            result_card, text="No password checked yet",
            font=("Segoe UI", 16, "bold"), text_color=theme.MUTED
        )
        self.pw_result_title.pack(anchor="w", padx=16, pady=(14, 2))

        self.pw_result_detail = ctk.CTkLabel(
            result_card, text="", font=("Segoe UI", 12), text_color=theme.MUTED,
            wraplength=560, justify="left"
        )
    ...
    def check_password_clicked(self):
        ...
        if not password:
            self.pw_status.configure(text="Enter a password first.", text_color=theme.DANGER)
            return

        self.pw_check_btn.configure(state="disabled", text="Checking...")
        self.pw_status.configure(text="Contacting Pwned Passwords API...", text_color=theme.MUTED)
    ...
    def _on_password_result(self, count, error):
        ...
        if error:
            self.pw_status.configure(text="Failed", text_color=theme.DANGER)
            self.pw_result_title.configure(text="⚠️ Lookup failed", text_color=theme.DANGER)
            self.pw_result_detail.configure(text=error)
            return

        self.pw_status.configure(text="Done", text_color=theme.SUCCESS)

        if count == 0:
            self.pw_result_title.configure(
                text="✅ Not found in any known breach", text_color=theme.SUCCESS
            )
            ...
            severity = theme.DANGER if count >= 100 else "#e0a030"
            self.pw_result_title.configure(
    ...
    def _build_email_tab(self):
        ...
        _btn(key_row, "Get a key", self._open_hibp_key_page,
             **theme.secondary_button_kwargs(), width=90).pack(side="left", padx=(8, 0))
        ...
        self.email_check_btn = _btn(btn_row, "Check Email", self.check_email_clicked, **theme.primary_button_kwargs())
    ...
    def _render_email_placeholder(self, text, color=None):
        ...
        ctk.CTkLabel(
            ...
            font=("Segoe UI", 12), text_color=color or theme.MUTED
    ...
    def check_email_clicked(self):
        ...
        if not email:
            self.email_status.configure(text="Enter an email first.", text_color=theme.DANGER)
            return
        if not key:
            self.email_status.configure(text="An HIBP API key is required.", text_color=theme.DANGER)
            return

        self.api_key.set(key)

        self.email_check_btn.configure(state="disabled", text="Checking...")
        self.email_status.configure(text="Contacting HIBP...", text_color=theme.MUTED)
    ...
    def _on_email_result(self, breaches, error):
        ...
        if error:
            self.email_status.configure(text="Failed", text_color=theme.DANGER)
            self._render_email_placeholder(f"⚠️ {error}", color=theme.DANGER)
            return

        if not breaches:
            self.email_status.configure(text="Done", text_color=theme.SUCCESS)
            self._render_email_placeholder("✅ No known breaches found for this address.", color=theme.SUCCESS)
            return
    ...
    def _render_breach_card(self, breach: dict):
        ...
        card = ctk.CTkFrame(self.email_results_scroll, fg_color=theme.PANEL_2, corner_radius=10)
        ...
        ctk.CTkLabel(
            top, text=title_text, font=("Segoe UI", 14, "bold"), text_color=theme.TEXT
        ).pack(side="left")
        ...
        if classes:
            ctk.CTkLabel(
                ...
                font=("Segoe UI", 11), text_color=theme.DANGER,
                wraplength=560, justify="left"
            ).pack(anchor="w", padx=14, pady=(0, 4))
        ...
        if description:
            ...
            ctk.CTkLabel(
                card, text=plain, font=("Segoe UI", 11), text_color=theme.MUTED,
                wraplength=560, justify="left"
