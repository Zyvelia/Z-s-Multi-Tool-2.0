            ...
            with open(tmp, "wb") as f:
                for chunk in resp.iter_content(chunk_size=256 * 1024):
                    ...
                    if total > 0:
                        ...
                        if pct - last_ui_pct >= 0.5:
                            last_ui_pct = pct
                            self.events.put(DownloadEvent(
                                kind="progress", downloaded=downloaded, total=total,
                            ))
                    else:
                        self.events.put(DownloadEvent(kind="progress", downloaded=downloaded, total=0))
...
# Sensible first-run defaults — Bedrock falls back to built-ins when
# server.properties is missing, and newer builds enable allow-list with
# no entries, which blocks every join attempt.
_BEDROCK_DEFAULT_PROPERTIES = {
...
def ensure_bedrock_server_properties(server_dir: Path) -> None:
    """Create or patch server.properties so new Bedrock servers are joinable."""
    server_dir.mkdir(parents=True, exist_ok=True)
    ...
    if not existing:
        lines = [
            ...
            "# Generated by Game Server Manager",
            "",
        ]
        lines.extend(f"{key}={value}" for key, value in _BEDROCK_DEFAULT_PROPERTIES.items())
        props_file.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return
    missing = {key: value for key, value in _BEDROCK_DEFAULT_PROPERTIES.items() if key not in existing}
    if missing:
        update_server_properties(server_dir, missing)
