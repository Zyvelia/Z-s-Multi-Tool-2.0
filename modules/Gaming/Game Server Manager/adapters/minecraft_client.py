...
from .minecraft_loaders import get_loader_versions
...
def _client_overrides_marker_path(client_dir: Path) -> Path:
    return client_dir / ".gsm_client_overrides_state.json"


def _client_overrides_already_applied(client_dir: Path, project_id: int, file_id: int) -> bool:
    marker = _client_overrides_marker_path(client_dir)
    if not marker.exists():
        return False
    try:
        state = json.loads(marker.read_text(encoding="utf-8"))
...
def _mark_client_overrides_applied(client_dir: Path, project_id: int, file_id: int) -> None:
    _client_overrides_marker_path(client_dir).write_text(
        json.dumps({"project_id": project_id, "file_id": file_id}), encoding="utf-8"
    )


def _resolve_project_id_by_name(api_key: str, name: str) -> int:
    """Best-effort: look up a CurseForge modpack's project ID from its name.

    Instances that were set up via "Import ZIP" or the manifest-less loader
    repair worker never had a CurseForge search+install step, so
    modpack_project_id is saved as 0 and stays 0 forever - which used to
    permanently block _ensure_client_overrides below (it bailed out
    immediately whenever project_id was falsy), even though the pack is a
    perfectly normal CurseForge project that does have a client overrides
    folder. This recovers the project ID from the pack's saved display name
    so those imported instances can still get their resourcepacks/
    CustomMainMenu assets instead of the magenta/black checkerboard.
    """
    name = (name or "").strip()
    ...
    try:
        data = _cf_json(
            f"{CURSEFORGE_API}/mods/search", api_key,
            params={
    ...
    # Require a reasonably close match so we don't attach the wrong pack's
    # cosmetics to this instance.
    if best is not None and best_ratio >= 0.6:
        return int(best.get("id") or 0)
    return 0


def _ensure_client_overrides(client_dir: Path, curseforge_api_key: str, project_id: int,
                              mc_version: str, file_id: int, pack_name: str = "") -> None:
    """Fetch client-only presentation assets from the CurseForge CLIENT pack.

    Dedicated server packs never include resourcepacks, shaderpacks, or
    things like CustomMainMenu's background images - CurseForge strips
    anything that isn't needed to run the world when building the server
    pack, including client-only mods, not just cosmetic asset folders.
    When the server's mods/ folder is already populated (installed
    from a proper dedicated server pack), Direct Launch used to skip
    touching CurseForge entirely and just mirror server-side folders,
    which never had these assets - leaving the main menu with Minecraft's
    magenta/black missing-texture checkerboard instead of the pack's
    intended background/animation, and any client-only mods (e.g. a
    custom loading-screen mod) silently, consistently missing.
    This fetches the CLIENT pack's overrides folder plus any manifest mod
    not already present in mods/, so those end up in client_dir too.
    Best effort: a failure here should never block launching the game.
    """
    if not project_id and curseforge_api_key and pack_name:
        project_id = _resolve_project_id_by_name(curseforge_api_key, pack_name)
    if not (curseforge_api_key and project_id):
        _write_client_overrides_debug(
            client_dir,
            f"Skipped: curseforge_api_key={'set' if curseforge_api_key else 'EMPTY'}, "
            f"project_id={project_id or 0} (name lookup for {pack_name!r} "
            f"{'found nothing' if curseforge_api_key and pack_name else 'was not attempted'})",
        )
        return
    try:
        resolved_file_id = file_id
        if not resolved_file_id or not _client_overrides_already_applied(client_dir, project_id, resolved_file_id):
            client_file = _find_client_file(curseforge_api_key, project_id, mc_version, file_id)
            resolved_file_id = int(client_file["id"])
        if _client_overrides_already_applied(client_dir, project_id, resolved_file_id):
            return
        client_url = _cf_download_url(curseforge_api_key, project_id, resolved_file_id)
        with tempfile.TemporaryDirectory(prefix="gsm_client_overrides_") as td:
            ...
            if manifest_path.exists():
                ...
                override = manifest_path.parent / override_name
                copied = 0
                if override.exists():
                    for src in override.rglob("*"):
                        if src.is_file():
                            ...
                            copied += 1
                # Cosmetic folders aren't the whole story - client-only mods
                # (see _add_missing_client_mods docstring) are missing too.
                _add_missing_client_mods(manifest, client_dir, curseforge_api_key)
                _write_client_overrides_debug(
                    client_dir,
                    f"OK: project {project_id} file {resolved_file_id}, "
                    f"override folder {'found' if override.exists() else 'MISSING from archive'} "
                    f"({override_name!r}), copied {copied} file(s).",
                )
            else:
                # No manifest at all (rare) - treat the whole archive as overrides.
                _copy_client_files(root, client_dir)
                _write_client_overrides_debug(client_dir, "OK: no manifest.json, copied whole archive as overrides.")
        _mark_client_overrides_applied(client_dir, project_id, resolved_file_id)
    except Exception as exc:
        # Cosmetic assets only - never block launch over this. But silently
        # swallowing every failure made this permanently undiagnosable (no
        # error, no log, nothing - just a checkerboard main menu with no
        # clue why). Leave a breadcrumb instead.
        _write_client_overrides_debug(client_dir, f"FAILED: {type(exc).__name__}: {exc}")


def _write_client_overrides_debug(client_dir: Path, message: str) -> None:
    try:
        client_dir.mkdir(parents=True, exist_ok=True)
        path = client_dir / ".gsm_client_overrides_debug.txt"
...
def _add_missing_client_mods(manifest: dict, client_dir: Path, api_key: str) -> None:
    """Install any CLIENT-manifest mod that isn't already in client_dir/mods.

    A dedicated server pack strips mods marked client-only, since they're
    pointless on a server (loading-screen customizers, menu backgrounds,
    shader-adjacent mods, etc.). Mirroring the server's mods/ folder into
    the client instance therefore permanently omits them - consistently,
    not randomly, since it's not a download failure, the mod was simply
    never there to copy. Diff against the CLIENT pack's manifest and fetch
    whatever's missing. Best effort per-mod: one mod CurseForge blocks from
    third-party download should never stop the rest from being added.
    """
    ...
    existing = {p.name.lower() for p in mods_dir.glob("*.jar")}
    for item in files:
        ...
        try:
            ...
            if not file_name or file_name.lower() in existing:
                continue
...
def _download(url: str, dest: Path) -> None:
    ...
    # dest directly. If the process is killed/crashes mid-write, a direct
    # write can leave a truncated file sitting at the final path, which the
    # asset-object checks above (and any other exists()-based cache checks)
    # would then treat as already-downloaded forever.
    tmp = dest.with_name(dest.name + f".tmp{os.getpid()}")
    tmp.write_bytes(data)
    os.replace(tmp, dest)


# Mojang has, over the years, quietly dropped a handful of old third-party
# libraries (jinput, jutils, lwjgl2, etc.) from libraries.minecraft.net.
# Old versions (1.12.2 and earlier) still reference them, so a fresh
# install of an old modpack/vanilla version can 404 on a library that the
# official launcher itself would also fail to fetch on a clean cache.
# These mirrors still host the full historical artifact set, so retry
# there before giving up.
_LIBRARY_MIRRORS = [
    "https://repo1.maven.org/maven2/{path}",
    "https://libraries.minecraft.net/{path}",  # retried in case of a transient blip
    "https://maven.minecraftforge.net/{path}",
]


def _download_library_with_fallback(url: str, dest: Path, path: str) -> None:
    """Download a library jar, retrying known mirrors if the primary URL 404s.

    `url` is the URL the version JSON actually points at (tried first,
    since it's authoritative when present). `path` is the maven-style
    relative path (group/artifact/version/artifact-version.jar) used to
    build mirror URLs as a fallback.
    """
    errors = []
    tried = set()
    candidates = [url] + [m.format(path=path) for m in _LIBRARY_MIRRORS]
    ...
    raise RuntimeError("; ".join(errors) if errors else f"no download URL available for {path}")
...
def _copy_client_files(server_dir: Path, client_dir: Path) -> None:
    for name in (
        ...
        # Some packs (SkyFactory 4 among them) use the ResourceLoader mod's
        # loose-folder resourcepacks instead of a zipped one in
        # resourcepacks/ - these were previously never copied at all.
        "resources", "CustomResources", "CustomOverridingResources",
    ):
        src = server_dir / name
        if not src.exists():
            continue
    ...
    if assets.get("id") and assets.get("url"):
        ...
        try:
            ...
            for obj in objects.values():
                ...
                # Not just existence: a crash, closed launcher, or dropped
                # connection mid-download can leave a 0-byte or truncated
                # file at `dest`. Once that happens this used to treat it as
                # "already downloaded" forever, permanently missing that
                # asset (e.g. a main-menu panorama background) with no way
                # to self-heal on a later launch. Comparing against the
                # size the asset index actually declares catches that.
                needs_download = not dest.exists()
                if not needs_download and isinstance(size, int):
                    try:
                        needs_download = dest.stat().st_size != size
                    except OSError:
                        needs_download = True
                if needs_download:
...
def _ensure_libraries(mc_dir: Path, libs: list[dict], natives_dir: Path) -> list[Path]:
    ...
    for lib in libs:
        ...
        if not path and not natives:
            # Only guess a maven path when the version JSON doesn't already
            # mark this as a natives-only library. Libraries like
            # jinput-platform / lwjgl-platform were NEVER published as a
            # plain classifier-less jar - only "<artifact>-natives-<os>.jar"
            # exists, on both Mojang's CDN and Maven Central. Synthesizing
            # "<artifact>-<version>.jar" for them always 404s since that
            # file has never existed anywhere.
            path = _maven_path(str(lib.get("name") or ""))
        if path:
            local = mc_dir / "libraries" / path
            if not local.exists():
                url = artifact.get("url") or f"https://libraries.minecraft.net/{path}"
                try:
                    _download_library_with_fallback(url, local, path)
                except Exception as exc:
            ...
            if classifier:
                ...
                if npath:
                    ...
                    if not native.exists() and native_info.get("url"):
                        _download_library_with_fallback(native_info["url"], native, npath)
                    if native.exists():
...
def prepare_client(
    ...
    *,
    ...
    # Imported/repaired instances never had a CurseForge search+install step,
    # so project_id was saved as 0. Recover it from the api key + pack name
    # up front so BOTH the full-install branch below (for a still-empty
    # mods/ folder) and the cosmetics-only branch can use it, not just the
    # cosmetics one.
    if curseforge_api_key and not project_id:
        project_id = _resolve_project_id_by_name(curseforge_api_key, profile_name)
    ...
    if curseforge_api_key and project_id and not _server_mods_populated(server_dir):
        ...
        # No mod re-download needed: either there's no project_id to look
        # up, or the server's mods folder is already populated (manually
        # copied in, or already downloaded by a prior server install) —
        # mirror those files straight into the client...
        _copy_client_files(server_dir, client_dir)
        # ...but a dedicated server pack never ships client-only cosmetic
        # assets (resourcepacks, shaderpacks, CustomMainMenu images, etc.),
        # so still fetch just the CLIENT pack's overrides for those.
        _ensure_client_overrides(client_dir, curseforge_api_key, project_id, mc_version, file_id,
                                  pack_name=profile_name)
...
def prepare_forge_client(
    ...
    *,
    ...
    # See the matching comment in prepare_client(): recover a missing
    # project_id from the api key + pack name up front so it's available to
    # both the full-install branch and the cosmetics-only branch below.
    if curseforge_api_key and not project_id:
        project_id = _resolve_project_id_by_name(curseforge_api_key, profile_name)
...
def _parse_manifest_loader(manifest: dict) -> tuple[str, str]:
    """(loader, loader_version) from a CurseForge manifest's modLoaders entry."""
...
def _prepare_loader_for_client(mc_dir: Path, mc_version: str, loader: str, loader_version: str,
                                java_path: str) -> str:
    """Install (if not already present) a client for the given loader and
    return its version id. Covers forge/neoforge/fabric/quilt/vanilla.

    This is used only by the client-only "play without a server" entry
    points below (prepare_client_from_zip / prepare_client_from_curseforge).
    prepare_client() and prepare_forge_client() have their own long-standing
    inline versions of this same dispatch and are deliberately left alone so
    nothing about the existing "convert an installed server into a client"
    flow changes.
    """
    loader = (loader or "vanilla").lower().strip()
    if loader == "forge":
        if not loader_version:
            raise RuntimeError("This modpack needs a Forge version, but none was found.")
        version_id = f"{mc_version}-forge-{loader_version}"
        version_json = mc_dir / "versions" / version_id / f"{version_id}.json"
        if not version_json.exists():
            url = (
                "https://maven.minecraftforge.net/net/minecraftforge/forge/"
    ...
    if loader == "neoforge":
        if not loader_version:
            raise RuntimeError("This modpack needs a NeoForge version, but none was found.")
        return _download_neoforge_client(mc_dir, mc_version, loader_version, java_path)
    if loader == "fabric":
        if not loader_version:
            raise RuntimeError("This modpack needs a Fabric loader version, but none was found.")
        return _install_fabric_client(mc_dir, mc_version, loader_version, java_path)
    if loader == "quilt":
        if not loader_version:
            raise RuntimeError("This modpack needs a Quilt loader version, but none was found.")
        return _install_quilt_client(mc_dir, mc_version, loader_version, java_path)
    return mc_version


def _infer_client_loader(root: Path, pack_name: str = "") -> tuple[str, str, str]:
    """Best-effort (mc_version, loader, loader_version) guess for a modpack
    ZIP with no manifest.json — just loose mods/config/etc. Looks only at
    the jar filenames already extracted from the ZIP, so this never needs
    a server install (unlike the equivalent guesswork used for manifest-less
    *server* pack imports).
    """
    ...
    neoforge_hits = sum(1 for t in texts if "neoforge" in t)
    forge_hits = sum(1 for t in texts if "forge" in t) - neoforge_hits
    fabric_hits = sum(1 for t in texts if "fabric" in t)
    quilt_hits = sum(1 for t in texts if "quilt" in t)
    if neoforge_hits >= max(forge_hits, fabric_hits, quilt_hits, 1):
        loader = "neoforge"
    elif forge_hits >= max(fabric_hits, quilt_hits, 1):
        loader = "forge"
    ...
    if loader != "vanilla" and mc_version:
        ...
        if error:
            raise RuntimeError(f"Couldn't determine the {loader.title()} version for Minecraft {mc_version}: {error}")
        if versions:
...
def prepare_client_from_zip(
    ...
    *,
    ...
    """Prepare a direct-launch client straight from a modpack ZIP on disk —
    with no dedicated server ever created, downloaded, or run.

    For someone who only wants to play a modpack themselves (singleplayer,
    or joining someone else's server) rather than host one. Installs
    directly into the client instance folder using the same manifest/
    loader-install machinery prepare_client() uses for its CurseForge
    download branch, just fed from the local ZIP instead.

    Returns (client_dir, version_id, mc_version, loader, loader_version) so
    the caller can persist those last three for a future re-launch.
    """
    zip_path = Path(zip_path)
    if not zip_path.exists():
        raise RuntimeError(f"Modpack archive not found: {zip_path}")
    ...
    with tempfile.TemporaryDirectory(prefix="gsm_client_zip_") as td:
        root = Path(td) / "root"
        root.mkdir()
        ...
        if manifest_path.exists():
            ...
            loader, loader_version = _parse_manifest_loader(manifest)
            if not mc_version:
                raise RuntimeError("This modpack ZIP's manifest has no Minecraft version.")
            needs_key = any(
                isinstance(item, dict) and item.get("projectID") and item.get("fileID")
                for item in (manifest.get("files") or [])
            )
            if needs_key and not curseforge_api_key.strip():
                raise RuntimeError(
                    "This modpack uses a CurseForge manifest. Enter a CurseForge API key to download its mods."
                )
            version_id = _prepare_loader_for_client(mc_dir, mc_version, loader, loader_version, java_path)
            _install_client_manifest(manifest_path.parent, manifest, client_dir, curseforge_api_key)
        else:
            # No manifest: a raw/share-profile ZIP with loose mods/config/etc.
            source = root
            if not (root / "mods").exists():
                subdirs = [p for p in root.iterdir() if p.is_dir()]
                if len(subdirs) == 1 and (subdirs[0] / "mods").exists():
                    source = subdirs[0]
            mc_version, loader, loader_version = _infer_client_loader(source, zip_path.stem)
            if not mc_version:
                raise RuntimeError(
                    "Could not determine the Minecraft version for this ZIP "
                    "(no manifest.json and no version could be inferred from its files)."
                )
            version_id = _prepare_loader_for_client(mc_dir, mc_version, loader, loader_version, java_path)
            _copy_client_files(source, client_dir)

    _write_profile(mc_dir, f"gsm-{instance_id}", profile_name, version_id, client_dir)
    return client_dir, version_id, mc_version, loader, loader_version


def _pack_website_url(api_key: str, project_id: int) -> str:
    """Best-effort CurseForge website link for a project, for error messages."""
    try:
        info = _cf_json(f"{CURSEFORGE_API}/mods/{project_id}", api_key).get("data") or {}
        return str((info.get("links") or {}).get("websiteUrl") or "").strip()
    except Exception:
        return ""


def prepare_client_from_curseforge(
    curseforge_api_key: str,
    project_id: int,
    java_path: str,
    profile_name: str,
    ...
    *,
    ...
    """Install + prepare a direct-launch client straight from a CurseForge
    modpack — with no dedicated server ever downloaded or run.

    Unlike prepare_client()/prepare_forge_client() (which convert an
    already-installed *server* pack into a client), this is the standalone
    entry point for someone who only ever wants to play the pack
    themselves: it resolves CurseForge's own CLIENT release directly, reads
    that release's manifest for its own Minecraft version/loader, and
    installs straight into the client instance folder.

    Returns (client_dir, version_id, mc_version, loader, loader_version) —
    the last three so the caller can persist them for a future re-launch via
    the normal prepare_client() path (which needs them supplied up front).
    """
    if not curseforge_api_key.strip():
        raise RuntimeError("A CurseForge API key is required to install this modpack.")
    if not project_id:
        raise RuntimeError("This CurseForge project has no valid ID.")

    client_file = _find_client_file(curseforge_api_key, project_id, mc_version_hint, file_id_hint)
    resolved_file_id = int(client_file["id"])
    ...
    try:
        ...
        # Some pack authors disable third-party/API downloads for their
        # CurseForge file entirely - the API then 403s instead of handing
        # back a link, and the only way to get the file is the website's
        # own download button. Give a clear, actionable message (with a
        # link when we can resolve one) instead of a raw HTTP error.
        website = _pack_website_url(curseforge_api_key, project_id)
        label = str(client_file.get("displayName") or client_file.get("fileName") or "this modpack")
        raise ModpackDownloadError(
            f"CurseForge blocked automatic download of {label} "
            f"(the author has disabled third-party downloads for this file). "
            f"Download it manually from CurseForge, then use Import ZIP "
            f"with \"Play only\" checked instead.",
            entries=[(label, website)] if website else [(label, "")],
        ) from exc
    with tempfile.TemporaryDirectory(prefix="gsm_client_pack_") as td:
        archive = Path(td) / "client-pack.zip"
        req = requests.get(client_url, headers={"User-Agent": UA}, timeout=120)
        ...
        if manifest_path.exists():
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            mc_version = str((manifest.get("minecraft") or {}).get("version") or mc_version_hint or "")
            loader, loader_version = _parse_manifest_loader(manifest)
            if not mc_version:
                raise RuntimeError("This modpack's manifest has no Minecraft version.")
            version_id = _prepare_loader_for_client(mc_dir, mc_version, loader, loader_version, java_path)
            _install_client_manifest(manifest_path.parent, manifest, client_dir, curseforge_api_key)
        else:
            mc_version = mc_version_hint or ""
            loader, loader_version = "vanilla", ""
            if not mc_version:
                raise RuntimeError("Could not determine this modpack's Minecraft version.")
            version_id = _prepare_loader_for_client(mc_dir, mc_version, loader, loader_version, java_path)
            _copy_client_files(root, client_dir)
