...
def create_backup_zip(server_dir: Path, name: str, keep: int = 5) -> Path:
    """Zip a server folder (skipping `_backups`) and prune old zips."""
    root = Path(server_dir)
    if not root.is_dir():
        raise FileNotFoundError(f"Server folder does not exist: {root}")
    dest_dir = root / "_backups"
    dest_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d_%H%M%S")
    safe = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in (name or "server"))
    zip_path = dest_dir / f"{safe}_{stamp}.zip"
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        for path in root.rglob("*"):
            if "_backups" in path.parts or not path.is_file():
                continue
