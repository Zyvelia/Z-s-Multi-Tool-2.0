...
admin.py
--------
Windows UAC elevation helpers for the Update Manager module. Self-contained
(no dependency on core/ internals) so it works whether the app is run from
source or as a frozen PyInstaller exe.

Blocking/unblocking the Windows Update services and uninstalling updates
both require admin rights (service control + wusa.exe). Elevation here is
on-demand: the user clicks "Restart as Administrator" only if/when an
action actually needs it, matching the convention in modules/cleaner/admin.py.
"""
