...
# The hosted/API-key chat (AITerminalModule) needs the `openai` package.
# That's a real dependency of the *Chat tab only* — Prompt Library and
# Local Model Runner need neither an API key nor this package. Import
# it defensively so a missing/broken `openai` install disables just the
# Chat tab instead of taking the whole AI Chat module (all three tabs)
# down with an ImportError at module-load time.
try:
    ...
    _HOSTED_CHAT_IMPORT_ERROR: str | None = None
    ...
    _HOSTED_CHAT_IMPORT_ERROR = str(e)

from .commands import parse, CommandRouter, HELP_TEXT
from .security import InMemorySecret
from .prompt_library import PromptLibraryUI
from .local_model_runner import LocalModelRunnerUI
...
class AIChatModule(ctk.CTkFrame):
    """
    Top-level frame for the AI Chat plugin entry — a tabview holding the
    original hosted/API-key chat (AITerminalModule, unchanged) plus two
    tabs that used to be separate top-level tools:

        💬 Chat           - AITerminalModule (this file, above)
        📚 Prompt Library  - prompt_library/ui.py
        🖥️ Local Models    - local_model_runner/ui.py

    Each tab is its own self-contained frame/class; this wrapper only
    handles layout. Splitting them into subfolders (rather than one
    giant file) keeps each concern - hosted API chat, saved prompts,
    local model serving - independently editable.
    """

    def __init__(self, container, manager=None, **kwargs):
        super().__init__(container, fg_color=COLOR_BG, **kwargs)
        self.manager = manager
        ...
        prompts_tab = self.tabview.add("📚 Prompt Library")
        local_tab = self.tabview.add("🖥️ Local Models")

        for tab in (chat_tab, prompts_tab, local_tab):
            tab.grid_rowconfigure(0, weight=1)
            tab.grid_columnconfigure(0, weight=1)

        self._build_chat_tab(chat_tab, manager)
        PromptLibraryUI(prompts_tab, manager).grid(row=0, column=0, sticky="nsew")
        LocalModelRunnerUI(local_tab, manager).grid(row=0, column=0, sticky="nsew")

    def _build_chat_tab(self, chat_tab, manager) -> None:
        """Isolated so a broken/missing `openai` install (the hosted
        chat's only real dependency) can't take Prompt Library or Local
        Model Runner down with it - it just disables this one tab."""
        if _HOSTED_CHAT_IMPORT_ERROR is not None:
            notice = ctk.CTkFrame(chat_tab, fg_color=COLOR_PANEL, corner_radius=8)
            notice.grid(row=0, column=0, sticky="nsew")
            notice.grid_rowconfigure(0, weight=1)
            notice.grid_columnconfigure(0, weight=1)
            ctk.CTkLabel(
                notice,
                text=(
                    "Hosted Chat is unavailable — the 'openai' package "
                    "didn't import.\n\n"
                    f"{_HOSTED_CHAT_IMPORT_ERROR}\n\n"
                    "Run:  pip install openai\n"
                    "then restart the app. Prompt Library and Local Models "
        ...
        try:
            AITerminalModule(chat_tab, manager).grid(row=0, column=0, sticky="nsew")
        except Exception as e:  # noqa: BLE001
            # Import succeeded but construction still failed for some
            # other reason (bad settings file, etc) - same isolation
            # principle applies at runtime, not just at import time.
            ctk.CTkLabel(
                chat_tab, text=f"Chat tab failed to load: {e}",
                text_color=COLOR_ERROR, wraplength=440, justify="center",
            ).grid(row=0, column=0, padx=20, pady=20)