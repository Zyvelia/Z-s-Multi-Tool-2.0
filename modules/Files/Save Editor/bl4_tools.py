...
class BL4Workspace(QDialog):
    ...
    def __init__(self, parent, sf, edit_callback: Callable[[str], None], record_callback: Callable[[str], None]):
        ...
        self._build_character(); self._build_currency(); self._build_inventory(); self._build_progression(); self._build_missions(); self._build_challenges()
    ...
    def _build_inventory(self):
        ...
        self.inventory_table = QTableWidget(0, 7); self.inventory_table.setHorizontalHeaderLabels(["Slot", "Item", "Type", "Level", "Rarity", "State", "Family"]); self.inventory_table.setAlternatingRowColors(True); self.inventory_table.setSelectionBehavior(QTableWidget.SelectRows); self.inventory_table.setSelectionMode(QTableWidget.SingleSelection); self.inventory_table.setSortingEnabled(True); self.inventory_table.itemSelectionChanged.connect(self._show_selected_item)
        ...
        actions = QHBoxLayout(); self.item_edit_btn = QPushButton("Edit Item…"); self.item_edit_btn.clicked.connect(self._edit_selected_item); self.item_edit_btn.setEnabled(False); actions.addWidget(self.item_edit_btn)
        ...
        self.item_remove_btn = QPushButton("Remove"); self.item_remove_btn.clicked.connect(self._remove_selected); self.item_remove_btn.setEnabled(False); actions.addWidget(self.item_remove_btn); actions.addStretch(1); ip.addLayout(actions)
    ...
    def _duplicate_selected(self):
        ...
        bp[new_slot] = {"serial": item.serial, "state_flags": item.state_flags}
        self._populate_inventory()
        for r in range(self.inventory_table.rowCount()):
    ...
    def refresh(self):
        current = self.tabs.currentIndex(); self.tabs.clear(); self._build_character(); self._build_currency(); self._build_inventory(); self._build_progression(); self._build_missions(); self._build_challenges(); self.tabs.setCurrentIndex(min(current, self.tabs.count() - 1))
