...
class ColorPickerModule(ctk.CTkFrame):
    ...
    def __init__(self, master, manager=None, **kwargs):
        ...
        self._rgb = hex_to_rgb(DEFAULT_COLOR)
    ...
    def _build_layout(self) -> None:
        ...
        self.preview = ctk.CTkLabel(
            left, text="", height=100, corner_radius=8, fg_color=DEFAULT_COLOR,
        )
        ...
        self.hex_var = ctk.StringVar(value=DEFAULT_COLOR)
        self.hex_entry = ctk.CTkEntry(hex_row, textvariable=self.hex_var, fg_color=PANEL_2)
        ...
        ctk.CTkButton(
            hex_row, text="Pick 🎯", width=70, fg_color=ACCENT, hover_color="#3d8fe0",
        ...
        # ---- RGB sliders ----
        ctk.CTkLabel(left, text="RGB", text_color=MUTED, font=ctk.CTkFont(weight="bold")).grid(
        ...
        # ---- HSV sliders ----
        ctk.CTkLabel(left, text="HSV", text_color=MUTED, font=ctk.CTkFont(weight="bold")).grid(
        ...
        ctk.CTkButton(
            copy_row, text="Copy HEX", fg_color=PANEL_2, hover_color=ACCENT,
            command=lambda: self._copy_text(self.hex_var.get()),
        ).pack(side="left", expand=True, fill="x", padx=(0, 6))
        ctk.CTkButton(
            copy_row, text="Copy RGB", fg_color=PANEL_2, hover_color=ACCENT,
            command=lambda: self._copy_text(f"rgb({self._rgb[0]}, {self._rgb[1]}, {self._rgb[2]})"),
        ).pack(side="left", expand=True, fill="x", padx=(6, 0))

        self.status_label = ctk.CTkLabel(left, text="", text_color=MUTED)
        ...
        ctk.CTkLabel(scheme_row, text="Palette:", text_color=MUTED).pack(side="left", padx=(0, 8))
    ...
    def _add_slider(self, parent, row: int, label: str, lo: int, hi: int, callback) -> ctk.DoubleVar:
        ...
        ctk.CTkLabel(row_frame, text=label, text_color=MUTED, width=16).grid(row=0, column=0)
        ...
        slider = ctk.CTkSlider(
            row_frame, from_=lo, to=hi, variable=var, progress_color=ACCENT,
            button_color=ACCENT, button_hover_color="#3d8fe0", command=on_change,
        )
    ...
    def _build_swatch(self, col: int, swatch_hex: str) -> None:
        ...
        swatch = ctk.CTkButton(
            ...
            fg_color=swatch_hex, hover_color=swatch_hex, border_width=1, border_color=PANEL_2,
        ...
        ctk.CTkButton(
            ...
            fg_color=PANEL_2, hover_color=ACCENT, text_color=MUTED,
